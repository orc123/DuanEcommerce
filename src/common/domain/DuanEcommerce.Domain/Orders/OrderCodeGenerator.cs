...
public class OrderCodeGenerator : ITransientDependency
{
    ...
    {
        string newCode;
        var identitySetting = await _identitySettingRepository.FindAsync(x => x.Id == DuanEcommerceConsts.OrderIdentitySettingId);
        if (identitySetting == null)
        {
            identitySetting = await _identitySettingRepository.InsertAsync(new IdentitySetting(DuanEcommerceConsts.OrderIdentitySettingId, "Đơn hàng", DuanEcommerceConsts.ProductIdentitySettingPrefix, 1, 1));
